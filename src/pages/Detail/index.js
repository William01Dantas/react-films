...
function Detail() {
    ...
    const [favoritesMovies, setFavoriteMovie] = useState(false);
    ...
    async function handlerFavoriteMovie(movie) {
        if (favoritesMovies) {
            await deleteMovie(movie.id);
            setFavoriteMovie(false)
            alert('Filme removido da sua lista');
        }else{
            await saveMovie('@reactfilms', movie)
    ...
    return(
        <Container>
            ...
            <Header>
                ...
                <HeaderButton onPress={()=> handlerFavoriteMovie(movie)}>
                    {favoritesMovies ? (
                        <Ionicons
                        name="bookmark"
                        size={28}
